...
declare function local:universal_pos_tag($node as element(node)) as attribute() {
  ...
  attribute ud:pos {
      ...
      if (matches($node/@root,'(om|te)')) then 'ADP'
      else 'SCONJ'
      ...
      else if (matches($node/@root,'klaar')) then 'ADJ' else 'NOUN'  
    else if ($POS eq 'prep')  then 'ADP'  (: v2: do not use PART for SVPs and complementizers :)
    else if ($POS eq 'part')  then 'ADP'
      ...
      (: if (matches($node/@root,'(en|maar|want|dus)'))  then  'CCONJ' :)
      (: else if (contains($node/@frame,'conj'))  then  'CCONJ' :)  
      (: else 'SCONJ' (: V2: CONJ ==> CCONJ :) :)
